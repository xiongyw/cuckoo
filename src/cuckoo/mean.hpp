...
#else
                            u32 u_pre = uxyz;
                            u32 u_cur = (u32)(e >> YZBITS); // uYZ with shorter Y-bit
                            u32 delta = (u_cur - u_pre) & SRCPREFMASK;
                            uxyz = u_pre + delta;
                            printf("%s(id=%d, round=%d): vx=%d, ux=%d, u_pre=%08x, u_cur=%08x, delta=%08x, uxyz=%08x\n", __FUNCTION__,
                                    id, round, vx, ux, u_pre, u_cur, delta, uxyz);
#endif
